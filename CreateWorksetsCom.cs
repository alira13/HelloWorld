...
namespace HelloWorld
{
    ...
    public class CreateWorksetsCom : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            string[,] excelData = ExcelFunc.ReadExcelFile(@"D:\Мой диск\BIM\C#\HelloWorld\RvtProjectConfigFile\RvtProjectConfigFile.xlsx", 3);
            ...
            for (int i = 2; i < excelData.GetLength(0); i++)
            {
                try
                {
                    ...
                    //TODO Ask user if document is not worckshared, EnableWorksharing or quit?
                    DocumentFunc.EnableWorksharing(newDoc, (string)excelData.GetValue(i, 1), (string)excelData.GetValue(i, 2));
